...
By default, backend connects to the host MySQL at `host.docker.internal:3306`, database `ai_english_exam`, user `root`, password `123`. Nginx is published on host port `8088` to avoid Windows/IIS/HTTP.sys bindings on `80`.
The backend service also loads `backend/.env` for provider secrets such as `APP_DEEPSEEK_API_KEY`; explicit Docker Compose values still override local-only values like `APP_AI_LOCAL_URL` so container traffic uses `http://aiservice:8002`.
