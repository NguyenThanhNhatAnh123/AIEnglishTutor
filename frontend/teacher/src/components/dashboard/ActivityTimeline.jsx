  ...
  if (!items.length) {
    return (
      <div className="flex min-h-72 flex-col items-center justify-center rounded-lg border border-dashed border-slate-200 bg-slate-50/70 text-center">
        ...
        <p className="mt-1 max-w-xs text-sm text-slate-500">New student attempts will appear here as a live teaching timeline.</p>
      </div>
    );
  }
          ...
          >
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <p className="truncate text-sm font-bold text-slate-950">{item.studentName || `Student #${item.studentId}`}</p>
                <span className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-bold ring-1 ${tone.className}`}>
                  <StatusIcon className="h-3.5 w-3.5" />
                  {item.status || 'UNKNOWN'}
                </span>
              </div>
              <p className="mt-1 truncate text-xs font-medium text-slate-500">{item.examTitle || `Exam #${item.examId}`}</p>
              <p className="mt-2 text-xs text-slate-400">{formatDateTime(item.endTime || item.submitTime || item.startTime)}</p>
            </div>
            <div className="flex items-center justify-between gap-3 sm:block sm:text-right">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Score</p>
